...
public class GithubService {
    ...
    public RepoDto getLocalRepo(String owner, String repoName) {
        return repoMapper.toDto(repository.getByOwnerAndRepoName(owner, repoName).orElseThrow(
                () -> new RepositoryNotFoundException(repoName)));
    }

    @Transactional
    public RepoDto postGithubRepo(String owner, String repoName) {
        if (repository.existsByOwnerAndRepoName(owner, repoName)) {
    ...
    public RepoDto putRepo(String owner, String repoName) {
        GithubRepo githubRepo = githubClient.getUserRepo(owner, repoName);
        Repo localRepo = repository.getByOwnerAndRepoName(owner, repoName).orElse(new Repo());
    ...
    public void deleteRepo(String owner, String repoName) {
        repository.getByOwnerAndRepoName(owner, repoName).orElseThrow(
                () -> new RepositoryNotFoundException(repoName));
        repository.deleteRepositoryByOwnerAndRepoName(owner, repoName);
    }
}